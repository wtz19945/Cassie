function s = DoublePendulumDescriptor(m1,m2)
  ...
  s(end).p  = bodyIndex(s, 'pendulum1'); % Index of parent link: pendulum 1
  s(end).r = [0.0 0.0 -1.0]; % Position of the link frame in the parent frame (in the negative z direction with length 1)
  s(end).R = eye(3); % Rotation of the link frame in the parent frame (Same as the world frame)
  s(end).m  = m2; % Mass
  s(end).c  = [0 0 -1]; % Center of mass position in the link frame, in the negative z direction with length 1
  s(end).Ic = [0 m2 m2 0 0 0]; % Inertia Matrix, for point mass is m*R^2 = m
  s(end).Sw = [1 0 0]; % Motion vector, rotation about x axis
  s(end).Sv = [0 0 0]; % Motion vector, no translation motion for this joint
