...
%                 [pos_lt,pos_rt] = GeneratedToeContactPoint(base,qls,qrs);
                ...
                if obj.is_sim == 1
                    if contact_switch(1) == 1
                        obj.y_offset = pel_pos(2);
                        obj.x_offset = pel_pos(1);
                    end
                    %                 orig_pel_traj = pel_pos_traj;
                    %                 orig_pel_traj(2) = orig_pel_traj(2);
                    if contact_switch(4) == 1
                        des_pelv_pos(1) = des_pelv_pos(1) + 1.6/2 * obj.x_offset;
                        des_pelv_pos(2) = des_pelv_pos(2) + 1.6/2 * obj.y_offset;
                    end
                else
                    if contact_switch(1) == 1
                        obj.y_offset = pel_pos(2);
                    ...
                    if contact_switch(4) == 1 % only update in stepping phase
                        des_pelv_pos(1) = des_pelv_pos(1) + 1.8/2 * obj.x_offset;
                        des_pelv_pos(2) = des_pelv_pos(2) + 1.8/2 * obj.y_offset;
                    end
                end
                


                % find desired foot acceleration based on 
%                 x_pgain = 2*15;
%                 x_dgain = 2*2;
%                 y_pgain = 2*15;
%                 y_dgain = 2*2;
...
%                 dy_e = -.0*(des_pelv_vel(2) - pel_vel(2));   
                ...
                if obj.globalTime <= 6
                    vel_des = [.0;0]; % Desired Robot States
                elseif obj.globalTime <= 18
                    vel_des = [min(.15*(obj.globalTime - 6),1.4);0]; % Desired Robot States
                else
                    vel_des = [max(1.4 - .1*(obj.globalTime - 18),-.5);0]; % Desired Robot States
                end
...
%                         footx_vel = 0;
                    ...
                    obj.yp = quintic_fitting(obj.foot_cur_y,obj.foot_des_y + [obj.foot_offy_left, 0, 0],Tstep - 0.1 * mid_in);
                    ...
                    des_foot = [x_des;dx_des;0;y_des;dy_des;ddy_des;contact;(pos_lh(1) + pos_lt(1))/2;(pos_lh(2) + pos_lt(2))/2;0];
...
%                         footx_vel = 0;
                    ...
                    obj.yp = quintic_fitting(obj.foot_cur_y,obj.foot_des_y + [obj.foot_offy_right, 0, 0],Tstep - 0.1 * mid_in);
...
%                 acc_x = (1 - lambda/200) * sec_QP(11) + lambda/200 * sec_QP(12);
                ...
                des_pelv_acc(2) = .3 * (1 - lambda/200) * sec_QP(13) + .3 * lambda/200 * sec_QP(14);
...
%                 end
                ...
                if contact(1) == 1
                    ...
                    [a,b] = traj_generate_walking(5,step_height,step_time,-.06,-0.3);
                    n = traj_time;
...
%                 end
                ...
                other_info = [G;dq;torque_lb;torque_ub;traj_time];
