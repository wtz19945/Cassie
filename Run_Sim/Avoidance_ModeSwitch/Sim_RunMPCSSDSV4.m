function [qd,flag,dual] = Sim_RunMPCSSDSV4(Tstep,Nodes,r_min,r_min2,cost,CoM_x,CoM_y,CP_x,CP_y,qo_i,states_ref,step_index...
   ...
   % Optimization Options
    ...
    coder.extrinsic('temp_MPCV92_form_Step0_mex');
    coder.extrinsic('temp_MPCV92_form_Step1_mex');
    coder.extrinsic('temp_MPCV92_form_Step2_mex');
    coder.extrinsic('temp_MPCV92_form_Step3_mex');
    ...
    if standing == 1
        switch MPC_index
            case 0
                [Aeq,beq,Aiq,biq,H,f] = temp_MPCV92_form_Step0_mex(states,Nodes,r,qo,states_ref,Foot_index,weights,foot_info);
                y_old = y_old0;
            case 1
                [Aeq,beq,Aiq,biq,H,f] = temp_MPCV92_form_Step1_mex(states,Nodes,r,qo,states_ref,Foot_index,weights,foot_info);
                y_old = y_old1;
            case 2
                [Aeq,beq,Aiq,biq,H,f] = temp_MPCV92_form_Step2_mex(states,Nodes,r,qo,states_ref,Foot_index,weights,foot_info);
                y_old = y_old2;
            case 3
                [Aeq,beq,Aiq,biq,H,f] = temp_MPCV92_form_Step3_mex(states,Nodes,r,qo,states_ref,Foot_index,weights,foot_info);
                y_old = y_old3;
            otherwise
                [Aeq,beq,Aiq,biq,H,f] = temp_MPCV92_form_Step0_mex(states,Nodes,r,qo,states_ref,Foot_index,weights,foot_info);
                y_old = y_old0;
        end
        temp = [zeros(8,102) eye(8) zeros(8,42)];
        Aeq = [Aeq;temp];
        beq = [beq;zeros(8,1)];
    else
        switch MPC_index
