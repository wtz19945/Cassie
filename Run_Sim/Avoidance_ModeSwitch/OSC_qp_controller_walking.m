function [res,flag] = OSC_qp_controller_walking(M,G,B,Jfl,Jfr,Js,A,Adot,xddot_cmd,dq,Jfldot,Jfrdot,x_old,acc_feet,contact,contact_lam,weights,torque_bound,qdd_range,traj_time)
...
max_force_left = 230 * contact(1);
max_force_right = 230 * contact(2);
...
if contact(1) == 0 && contact(2) == 1
    if traj_time <= 0.1
        ...
        contact_force_max(9)  = 30 + 180 * min(traj_time/0.1,1);
        contact_force_max(12) = 30 + 180 * min(traj_time/0.1,1);
...
if contact(2) == 0 && contact(1) == 1
    if traj_time <= 0.1
        ...
        contact_force_max(3) = 30 + 180 * min(traj_time/0.1,1);
        contact_force_max(6) = 30 + 180 * min(traj_time/0.1,1);
    end
end
