...
classdef Traj_generator_withMPCV4_StepEnd < ...
        matlab.System & ...
    ...
    methods (Access = protected)
        ...
        function setupImpl(obj)
            ...
            obj.step_index = -1;
            obj.start_foot = 0;
            obj.result_old = zeros(152 ,1);
            obj.result_old2 = zeros(152 ,1);
            obj.result_y_old0 = zeros(216,1);
            obj.result_y_old1 = zeros(213,1);
            obj.result_y_old2 = zeros(210,1);
            obj.result_y_old3 = zeros(207,1);
            
            obj.result_y_old01 = zeros(224,1);
            obj.result_y_old11 = zeros(221,1);
            obj.result_y_old21 = zeros(218,1);
            obj.result_y_old31 = zeros(215,1);
            
            obj.MPC_Info = zeros(15,1);
            obj.MPC_index = 0;
            obj.MPC_Switch = 1;
            obj.MPC_Command = zeros(4,1);
            obj.MPC_SwitchBack = zeros(10,1);
...
%             obj.x_acc = temp.ddyData;
        ...
        function [des_pos, des_vel,des_acc,contact,contact_switch, traj_time, sc_Data, MPC_Slack, MPC_Data] = stepImpl(obj, cassieOutputs, stepping_command, state)
            ...
            if cassieOutputs.isCalibrated
                ...
                if sum(obj.MPC_SwitchBack(1:9)) == 9 && obj.MPC_SwitchBack(10) == 0 && sc == 0
                    obj.MPC_SwitchBack(10) = 1;
                    obj.MPC_Switch = 1;
                end
                
%                 if obj.globalTime >= 7.0
%                     sc = 1;
%                 end
                ...
                if obj.stepping == 1 || obj.step_end == 1
                    ct = obj.traj_time;
                    if ct<=step_time
                        if obj.start_foot == 1
                            contact = [0;1];
                ...
                qo_i = [obj.x_obs_int;1.5] + [0; -.25 * (obj.globalTime)];
%                     qo_i = [1;1];
                ...
                if standing == 0
                    ...
                    y_ref = [0;0];
%                     if obj.globalTime >= 6
%                         x_ref(2) = min(.1 * (obj.globalTime - 6),1.6);
%                     end
...
%                 r_min2 = 0.6; % Soft Constraint Radius
                r_min = 0.2; % Hard Constraint Radius,
                r_min2 = 1.0; % Soft Constraint Radius
                cost = 2500; % Soft Constraint Cost (outter circle penalty), can modify here
                cost_step = 500;
                Qx_st = .3 * [15000;100];
                Qx_sw = [0;500];
                Qy_st = .3 * [15000;100];
...
%                 end
                ...
                if obj.step_start == 1 
                    state_desst = [.0;0;-.3 * .2;-.2];
                    CoM_xst = CoM_x;
                    CoM_yst = CoM_y;
                elseif obj.stepping == 1
                    state_des = [0;-.2 * CoM_x(1);0;-.2 * CoM_y(1)];
...
%                     Cp_yst = (pos_rh(2) + pos_rt(2) + pos_lh(2) + pos_lt(2))/4;
                ...
                if mod(traj_time,0.1) <= 5e-4 && traj_time <=0.39
                    ...
                    [qd,flag,sol_y] = Sim_RunMPCSSDSV4(Tstep,Nodes,r_min,r_min2,cost,CoM_xst,CoM_yst,Cp_xst,Cp_yst,qo_i,state_desst,obj.step_index,...
                    ...
                    obj.result_old = qd;
                    norm2 = norm(obj.result_old(149:152),2);
                    norm1 = norm(obj.result_old(145:148),2);
...
%                     if obj.globalTime >= 3
                    ...
                    if obj.stepping ~= 1
                        obj.MPC_SwitchBack(1:8,1) = zeros(8,1);
                    else
                        obj.MPC_SwitchBack(1:8) = obj.MPC_SwitchBack(2:9);
                        if norm2  < .008 && norm1 < 0.01
                            obj.MPC_SwitchBack(9) = 1;
                        else
                            obj.MPC_SwitchBack(9) = 0;
                        end
                    end
                    
                    obj.MPC_Info(4:6,1) = [norm([CoM_x(1);CoM_y(1)] - qo_i); norm(qd(145:148),2); norm(qd(149:152,1),2)];
                    obj.MPC_Info(7:14) = [CoM_x;CoM_y;Cp_xst;Cp_yst;qo_i];
                end
                if standing == 1
                    obj.MPC_Command(4) = 1;
                end
               ...
               if contact(1) == 0
                    ...
                    if mod(traj_time,0.1) <= 5e-4 && traj_time <=0.39
                        [qd,flag,sol_y] = Sim_RunMPCSSDSV4(Tstep,Nodes,r_min,r_min2,cost_step,CoM_x,CoM_y,Cp_x,Cp_y,qo_i,state_des,obj.step_index,...
...
%                         foot_info(end) = mid_in;
                ...
                MPC_Data = [des_traj(1:68);Cp_x;Cp_y;des_traj(103 : 105,1);des_traj(107 : 109,1)];
                sc_Data = [zeros(42,1);des_traj(103);des_traj(107);des_traj(1:4,1);des_traj(35:38);acc_des];
                if obj.step_end == 1
                    if contact(1) == 1
                        sc_Data(44) = sc_Data(44) - 0.05;
                        sc_Data(43) = sc_Data(43) + 0.0;
                    end
                end
            end
