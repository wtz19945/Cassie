...
% 2020 Tianze Wang
classdef Sim_CassieEKF_Bias < matlab.System & matlab.system.mixin.Propagates %#codegen
    ...
    methods (Access = protected)
        ...
        function [pel_pos_estimation,pel_vel_estimation,other] = stepImpl(obj, cassieOutputs, ...
            ...
            if cassieOutputs.isCalibrated
                ...
                qsL = .5 * qsL + .5 * obj.qs_prev(1:2,1);
                qsR = .5 * qsR + .5 * obj.qs_prev(3:4,1);
                obj.qs_prev = [qsL;qsR];
                ...
                if obj.filter_initialized
                    ...
                    if contact(2) == 1 && contact(1) == 1
                        pel_pos_estimation(3) = pel_pos_estimation(3) - ((obj.n(9,1) + obj.n(12,1))/2 + + 0.07);
                    elseif contact(2) == 1
                        pel_pos_estimation(3) = pel_pos_estimation(3) - (obj.n(12,1) + + 0.07);
                    elseif contact(1) == 1
                        pel_pos_estimation(3) = pel_pos_estimation(3) - (obj.n(9,1) + + 0.07);
                    else
                        pel_pos_estimation = obj.n(1:3,1);
                    end
...
%             R_pred = R * expm(obj.skew(w*dt));
            ...
            swing_foot_noise_Q = 0 * blkdiag(zeros(3,3),zeros(3,3),obj.Qc+1e4*eye(3)*(1-lm1),obj.Qc+1e4*eye(3)*(1-lm2),zeros(3,3));
        ...
        function [error] = Update_ForwardKinematics(obj, encoders, contact,pos_lf,pos_rf,R,J_lf,J_rf)
            ...
            if contact(2) == 1 && contact(1) == 1
                % Double Support
                C = [-R' zeros(3,3) R' zeros(3,3) zeros(3,3);-R' zeros(3,3)  zeros(3,3) R' zeros(3,3)];
...
%                 obj.Q = obj.Q - K*C*obj.Q;
            ...
            elseif contact(2) == 1 && contact(1) == 0
                % Double Support
                C = [-R' zeros(3,3)  zeros(3,3) R' zeros(3,3)];
...
%                 obj.Q = obj.Q - K*C*obj.Q;
            ...
            elseif contact(1) == 1 && contact(2) == 0
                % Double Support
                C = [-R' zeros(3,3) R' zeros(3,3) zeros(3,3)];
