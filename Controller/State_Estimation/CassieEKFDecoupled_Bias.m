...
% 2020 Tianze Wang
classdef CassieEKFDecoupled_Bias < matlab.System & matlab.system.mixin.Propagates %#codegen
...
%                     pel_vel_estimation = pel_pos_vel(4:6,1);
                    other(1:2,1) = contact;
                    other(3:4,1) = [(abs(qsL(1)) + abs(qsL(2)))/2; (abs(qsR(1)) + abs(qsR(2)))/2];
                    other(5:6,1) = obj.nz(3:4,1);
                else
